...
async def who(event):
    ...
    await event.edit(
        "`Tenang sementara saya mencuri beberapa data dari *Global Network Zone*...`"
    )
    ...
    try:
        ...
        return await event.edit("`Tidak dapat mengambil info dari pengguna itu.`")
...
async def fetch_info(replied_user, event):
    ...
    replied_user_profile_photos_count = (
        "Orang ini membutuhkan bantuan dengan mengunggah gambar profil."
    )
    try:
    ...
    caption += f"Bot atau bukan: {is_bot}\n"
    caption += f"Is Dibatasi: {restricted}\n"
    caption += f"Diverifikasi oleh Telegram: {verified}\n"
    caption += f"ID: <code>{user_id}</code>\n\n"
    caption += f"Bio: \n<code>{user_bio}</code>\n\n"
    caption += f"Obrolan Umum: {common_chat}\n"
    caption += "Tautan profile tetap: "
